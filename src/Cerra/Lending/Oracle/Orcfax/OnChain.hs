...
import Plutus.V2.Ledger.Api (TxOut, TxInfo, BuiltinByteString, POSIXTime, txInfoValidRange)
import PlutusTx.AssocMap as Map
import Cerra.Lending.Oracle.Orcfax.Types (OrcfaxOracleDatum (..))
import Cerra.Lending.Utils.Utils (fromJustCustom, getLowerBound, getUpperBound, adaCoin)
import Cerra.Lending.Utils.OnChainUtils (mustFindScriptDatum)
import Ledger (AssetClass)

import qualified PlutusTx

import PlutusTx.Builtins (subtractInteger, multiplyInteger, addInteger, divideInteger)
...
getOrcfaxPrice info oracleInput =
    ...
    in if Internals.decodeUtf8 (Internals.unsafeDataAsB priceName) == "ADA-USD|USD-ADA"
            ...
            && upperBound <= validTo
        then (adjustedPrice, (adaCoin, "orcfax")) else debugError "E7" True